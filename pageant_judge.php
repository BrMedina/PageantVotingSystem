...
$role = $_SESSION['role'] ?? 'judge';
$isAdmin = ($role === 'admin');
?>
<!-- Pageant ADMIN > Judge page -->
<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <nav class="navbar navbar-expand-lg bg-dark navbar-dark sticky-top shadow-lg">
    <div class="container-fluid">
      ...
      <!-- logo pic -->
        <a class="navbar-brand" href="pageant_home.php">
        <img src="imagespvs/logo2.png" alt="logo" width="35" height="25">
        </a>

      <a class="navbar-brand" href="pageant_home.php">CrownVote</a>
  ...
  <div class="d-flex">
    ...
    <img src="imagespvs/user.png" alt="User" width="40" height="40" class="rounded-circle me-2">
  ...
  <hr>
        ...
        <ul class="nav nav-pills flex-column mb-auto">
            ...
            <!-- Judge -->
          <li class="nav-item"><a href="pageant_judge.php" class="nav-link text-white active">
    ...
    <main class="content-wrapper p-5 bg-light">
      ...
      <div class="p-4 bg-yellow shadow text-white justify-content-center align-items-center">
        <h1 class="display-3 fw-bold">Judge Accounts</h1>
         <p class="lead">Manage Judge Accounts</p>
        </div>

      <br>
      
      <!-- Display Table ==== INSERT TABLE BELOW ==== -->
      <form action="pageant_judge.php" method="post">
          <div class="row g-3">
              <div class="col">
...
// ensure judge accounts exist for users promoted to judge
$syncSql = "INSERT INTO tbl_judge (judge_name, contact_information, user_id)
      SELECT u.fullname, u.email, u.user_id
      FROM tbl_users u
      LEFT JOIN tbl_judge j ON j.user_id = u.user_id
      WHERE u.role = 'judge' AND u.status = 'Active' AND j.user_id IS NULL";
$conn->query($syncSql);
...
$displaysql = "SELECT
    j.judge_id,
    COALESCE(j.judge_name, u.fullname) AS judge_name,
    COALESCE(j.contact_information, u.email) AS contact_information,
    COALESCE(u.img_path, 'imagespvs/user.png') AS judge_image
  FROM tbl_judge j
  LEFT JOIN tbl_users u ON u.user_id = j.user_id
  WHERE (j.user_id IS NULL OR (u.role = 'judge' AND u.status = 'Active'))";

if ($searchinput !== '') {
  $displaysql .= " AND (j.judge_id LIKE '%".$searchinput."%'
    OR COALESCE(j.judge_name, u.fullname) LIKE '%".$searchinput."%'
    OR COALESCE(j.contact_information, u.email) LIKE '%".$searchinput."%')";
}
...
if ($result->num_rows > 0) {
    ...
    foreach ($result as $index => $fieldname) {
      $judgeImage = "imagespvs/user.png";
      if (isset($fieldname['judge_image'])) {
        $judgeImage = $fieldname['judge_image'];
      }
?>
    ...
    <div class="col-4">
        <div class="container p-3 m-3 bg-secondary text-white rounded shadow">
            ...
            <div class="row">
              ...
              <img src="<?php echo $judgeImage; ?>" alt="" width="200" height="150" class="img-fluid rounded">
            </div>
        </div>
        
            <div class="row">
                <div class="col">
                    <h5><?php echo $fieldname['judge_id'] ?></h5>
                </div>
            </div>

            <div class="row">
                <div class="col">
                        <?php echo $fieldname['judge_name'] ?>
                </div>
            </div>

            <div class="row">
                <div class="col">
                        <?php echo $fieldname['contact_information'] ?>
                </div>
            </div>
