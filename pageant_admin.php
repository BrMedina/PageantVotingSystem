  ...
  <hr>
        ...
        <ul class="nav nav-pills flex-column mb-auto">
          ...
          <li class="nav-item"><a href="pageant_judge.php" class="nav-link text-white">
            ...
            <!-- Admin -->
          <li><a href="pageant_admin.php" class="nav-link text-white active">
    ...
    <main class="content-wrapper p-5 bg-light">
      ...
      <div class="p-4 bg-yellow shadow text-white justify-content-center align-items-center">
        <h1 class="display-3 fw-bold">Admin Accounts</h1>
         <p class="lead">Manage Admin Accounts</p>
        </div>  
      <br>
      
      <!-- Display Table ==== INSERT TABLE BELOW ==== -->
      <form action="pageant_admin.php" method="post">
          <div class="row g-3">
              <div class="col">
...
if (isset($_POST['btnsearch'])) {
    ...
    if ($searchinput != NULL) {
      ...
      $displaysql = "SELECT * FROM tbl_users WHERE role = 'admin' AND (
      fullname LIKE '%".$searchinput."%'
      ...
      $displaysql = "SELECT * FROM tbl_users WHERE role = 'admin'";
...
if ($result->num_rows > 0) {
    ...
    foreach ($result as $index => $fieldname) {
      $adminImage = "imagespvs/user.png";
      if (isset($fieldname['img_path'])) {
        $adminImage = $fieldname['img_path'];
      }
      $adminId = '';
      $adminName = '';
      $contactInfo = '';
      if (isset($fieldname['user_id'])) {
        $adminId = $fieldname['user_id'];
      }
      if (isset($fieldname['fullname'])) {
        $adminName = $fieldname['fullname'];
      }
      if (isset($fieldname['email'])) {
        $contactInfo = $fieldname['email'];
      }
?>
