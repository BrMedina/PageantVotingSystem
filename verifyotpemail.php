...
function send_verification($fullname, $email, $otp){
    ...
    try {
        ...
        $mail->SMTPAuth   = true;
        $mail->Username   = 'brentcarlo.medina.cics@ust.edu.ph'; 
        $mail->Password   = 'piyi yxmv xvud xoio'; 
        $mail->SMTPSecure = 'tls';
        $mail->Port       = 587;

        // Recipients
        $mail->setFrom('brentcarlo.medina.cics@ust.edu.ph', 'Crown Vote');
...
<p>Thank you for signing up at <strong>Pageant Voting Systems</strong>.</p>
...
<div style="background-color: #f8f9fa; padding: 15px; border-radius: 5px; text-align: center; font-size: 24px; color: #004aad; font-weight: bold;">
        '.$otp.' </div>
<p style="margin-top:10px;font-size: 14px; color: #6c757d;">— Crown Vote</p>';
